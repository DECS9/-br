...
@dashboard.route("/Propiedades/delete/<int:id>")
@login_required
def delete(propId):
    # que pasos tengo hacer para eliminar el prop id
    currentProp = Property.query.filter_by(id=propId).first()
    db.session.delete(currentProp)
    db.session.commit()
